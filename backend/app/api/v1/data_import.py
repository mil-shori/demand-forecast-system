...
async def import_orders(
    ...
    if not file.filename.lower().endswith((".csv", ".xlsx")):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="CSVまたはExcelファイルのみサポートしています"
        )
    ...
    try:
        ...
        for _, row in df.iterrows():
            # 既存データチェック
            existing_order = (
                db.query(Order).filter(Order.order_id == row["order_id"]).first()
            )
...
async def import_order_items(
    ...
    try:
        ...
        return {
            ...
            "summary": {"total_rows": len(df), "inserted": inserted_count},
            "validation": validation_result,
        }
...
async def import_products(
    ...
    try:
        ...
        for _, row in df.iterrows():
            existing_product = (
                db.query(Product).filter(Product.sku == row["sku"]).first()
            )
