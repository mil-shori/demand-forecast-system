...
from app.services.forecast_engine import SimpleForecastEngine, ForecastConfig, ForecastResult
...
async def run_forecast(
    ...
    try:
        ...
        background_tasks.add_task(
            _execute_forecast_background,
            forecast_id, request, db
        )
...
async def _execute_forecast_background(
    ...
    try:
        ...
        # 注文明細データ取得
        order_items_data = pd.read_sql(
            db.query(OrderItem).statement, db.bind
        )
        
        # 商品データ取得
        products_data = pd.read_sql(
            db.query(Product).statement, db.bind
        )
        ...
        processed_data = preprocessor.preprocess_for_forecasting(
            orders_data, order_items_data, products_data,
        ...
        logger.info(f"予測完了: {forecast_id}, SKU数: {successful_forecasts}, 処理時間: {execution_time:.2f}秒")
        ...
        "forecast": [
            {
                ...
                "confidence_intervals": {
                    level: {
                        "lower": intervals["lower"][i] if i < len(intervals["lower"]) else None,
                        "upper": intervals["upper"][i] if i < len(intervals["upper"]) else None
                    }
