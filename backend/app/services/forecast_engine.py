...
class SKUClassifier:
    ...
    @staticmethod
    def classify_sku(
        demand_data: pd.Series, sku_metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """SKUの特性分析とモデル推奨"""
        ...
        classification = {
            ...
            "intermittency": 1 - (demand_periods / total_periods)
            if total_periods > 0
            else 1,
            "average_demand": non_zero_demand.mean() if len(non_zero_demand) > 0 else 0,
            "demand_variability": non_zero_demand.std()
        ...
        if classification["intermittency"] > 0.75:
            ...
            classification["recommended_models"] = [
...
class SimpleForecastEngine:
    ...
    def _forecast_single_sku(
        self, sku_data: pd.DataFrame, sku_metadata: Dict[str, Any], horizon: int
    ) -> ForecastResult:
        """単一SKUの予測"""
        ...
        return ForecastResult(
            ...
            if isinstance(best_model, ForecastModel)
            else str(best_model),
    ...
    def _prepare_timeseries_data(self, sku_data: pd.DataFrame) -> pd.DataFrame:
        ...
        date_range = pd.date_range(
            start=ts_data["ds"].min(), end=ts_data["ds"].max(), freq="D"
        )
    ...
    def _select_best_model(
        self, classification: Dict[str, Any], ts_data: pd.DataFrame
    ) -> ForecastModel:
        """最適モデルの選択"""
        ...
        if len(ts_data) < 30:
            ...
            if ForecastModel.AUTO_ARIMA in recommended_models:
                ...
                return (
                    ...
                    else ForecastModel.AUTO_ETS
                )

    def _run_statsforecast(
        self, ts_data: pd.DataFrame, model: ForecastModel, horizon: int
    ) -> Dict[str, Any]:
        """StatsForecastでの予測実行"""
        ...
        try:
            ...
            for level in self.config.confidence_levels:
                ...
                if (
                    lo_col in forecasts_with_intervals.columns
                    and hi_col in forecasts_with_intervals.columns
                ):
                    intervals[str(level)] = {
    ...
    def _run_fallback_forecast(
        self, ts_data: pd.DataFrame, horizon: int
    ) -> Dict[str, Any]:
        """フォールバック予測（シンプルな手法）"""
        ...
        forecast_dates = (
            pd.date_range(
                start=ts_data["ds"].max() + timedelta(days=1), periods=horizon, freq="D"
            )
            .strftime("%Y-%m-%d")
            .tolist()
        )
        ...
        return {"dates": forecast_dates, "values": forecasts, "intervals": intervals}

    def _get_sku_metadata(
        self, sku: str, sku_metadata: Optional[pd.DataFrame]
    ) -> Dict[str, Any]:
        """SKUメタデータの取得"""
        ...
        # デフォルト値
        return {"sku": sku, "category": "unknown", "is_set_item": False}

    def _fallback_forecast(self, sku: str, horizon: int) -> ForecastResult:
        """エラー時のフォールバック予測"""

        forecast_dates = (
            pd.date_range(start=datetime.now().date(), periods=horizon, freq="D")
            .strftime("%Y-%m-%d")
            .tolist()
        )
    ...
    def calculate_accuracy_metrics(
        ...
        if mask_nonzero.any():
            metrics["mape"] = (
                np.mean(
                    np.abs(
                        (actual_clean[mask_nonzero] - predicted_clean[mask_nonzero])
        ...
        if actual_clean.sum() != 0:
            metrics["wmape"] = (
                np.sum(np.abs(actual_clean - predicted_clean)) / np.sum(actual_clean)
            ) * 100
        else:
