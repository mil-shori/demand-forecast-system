    ...
    def _apply_validation_rule(self, df: pd.DataFrame, rule: ValidationRule) -> Dict[str, Any]:
        ...
        if rule.rule_type == "required":
            ...
            if "min" in rule.parameters:
                ...
                if invalid_count > 0:
                    messages.append(f"{rule.field}: {invalid_count}件が最小値({min_val})未満です")
            
            if "max" in rule.parameters:
                max_val = rule.parameters["max"]
                invalid_count = (series > max_val).sum()
                if invalid_count > 0:
                    messages.append(f"{rule.field}: {invalid_count}件が最大値({max_val})を超えています")
        ...
        if data_type == "orders":
            if "order_date" in df.columns:
                try:
                    ...
                    stats["date_range"] = {
                        "min_date": dates.min().strftime("%Y-%m-%d") if dates.min() else None,
                        "max_date": dates.max().strftime("%Y-%m-%d") if dates.max() else None
                    }
                except:
                    pass
            
            if "order_type" in df.columns:
                stats["order_type_distribution"] = df["order_type"].value_counts().to_dict()
                
            if "total_amount" in df.columns:
                amounts = pd.to_numeric(df["total_amount"], errors='coerce')
                stats["amount_statistics"] = {
                    "mean": round(amounts.mean(), 2) if not amounts.isna().all() else None,
                    "median": round(amounts.median(), 2) if not amounts.isna().all() else None,
                    "min": round(amounts.min(), 2) if not amounts.isna().all() else None,
                    "max": round(amounts.max(), 2) if not amounts.isna().all() else None
                }
            ...
            if "is_set_item" in df.columns:
                stats["set_item_distribution"] = df["is_set_item"].value_counts().to_dict()
...
class DataPreprocessor:
    ...
    def _merge_order_data(
        ...
        merged = pd.merge(
            merged,
            products_df[["sku", "product_name", "category", "subcategory", "is_set_item"]],
            on="sku",
            how="left"
        )
    ...
    def _create_daily_demand_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # SKU × 日付別の需要集計
        daily_demand = df.groupby(["sku", "order_date"]).agg({
            "quantity": "sum",
            "order_id": "nunique",
            "total_amount": "sum",
        ...
        daily_demand.columns = [
            "sku", "ds", "y", "order_count", 
            "revenue", "subscription_count", "category", "subcategory"
        ]
    ...
    def _create_sku_metadata(
        ...
        sku_stats = merged_df.groupby("sku").agg({
        ...
        # カラム名の平坦化
        sku_stats.columns = [
            "sku", "total_quantity", "avg_quantity", "std_quantity", "order_frequency",
            "first_order_date", "last_order_date", "subscription_ratio", "avg_order_value"
        ]
        
        # 商品マスタ情報をマージ
        sku_metadata = pd.merge(
            sku_stats,
            products_df,
            on="sku",
            how="left"
