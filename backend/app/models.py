...
class Product(Base):
    ...
    set_components = relationship(
        "ProductSet", foreign_keys="ProductSet.set_sku", back_populates="set_product"
    )
    component_of_sets = relationship(
        "ProductSet",
        foreign_keys="ProductSet.component_sku",
...
class ProductSet(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    set_sku = Column(
        String(100), ForeignKey("products.sku"), nullable=False, index=True
    )
    ...
    # リレーション
    set_product = relationship(
        "Product", foreign_keys=[set_sku], back_populates="set_components"
    )
    component_product = relationship(
        "Product", foreign_keys=[component_sku], back_populates="component_of_sets"
    )
...
class UserProfile(Base):
    ...
    __table_args__ = (Index("ix_user_profiles_segment", "customer_segment"),)
...
class JournalEntry(Base):
    ...
    )  # 'sales_daily' | 'sales_monthly' | 'expense'
...
class Expense(Base):
    ...
    status = Column(
        SQLEnum(ExpenseStatus), default=ExpenseStatus.DRAFT, nullable=False, index=True
    )
