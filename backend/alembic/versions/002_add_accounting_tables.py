...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_freee_tokens_company_id'), 'freee_tokens', ['company_id'], unique=True)
    op.create_index(op.f('ix_freee_tokens_is_active'), 'freee_tokens', ['is_active'], unique=False)

    # Create account_item_mappings table
    op.create_table('account_item_mappings',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('mapping_type', sa.Enum('SALES_CATEGORY', 'EXPENSE_CATEGORY', name='mappingtype'), nullable=False),
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('mapping_type', 'source_key', name='uq_account_item_mappings_type_key')
    )
    op.create_index(op.f('ix_account_item_mappings_mapping_type'), 'account_item_mappings', ['mapping_type'], unique=False)
    op.create_index(op.f('ix_account_item_mappings_active'), 'account_item_mappings', ['active'], unique=False)

    # Create journal_entries table
    op.create_table('journal_entries',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('entry_date', sa.Date(), nullable=False),
        sa.Column('entry_type', sa.Enum('SALES', 'EXPENSE', name='journalentrytype'), nullable=False),
        ...
        sa.Column('status', sa.Enum('PENDING', 'SYNCED', 'FAILED', 'SKIPPED', name='journalentrystatus'), nullable=False),
        sa.Column('error_message', sa.Text(), nullable=True),
        sa.Column('synced_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('source_type', 'source_key', name='uq_journal_entries_source')
    )
    ...
    op.create_index(op.f('ix_journal_entries_freee_deal_id'), 'journal_entries', ['freee_deal_id'], unique=False)
    op.create_index(op.f('ix_journal_entries_status'), 'journal_entries', ['status'], unique=False)
    op.create_index('ix_journal_entries_date_status', 'journal_entries', ['entry_date', 'status'], unique=False)
    ...
    op.create_table('expenses',
        ...
        sa.Column('status', sa.Enum('DRAFT', 'CONFIRMED', 'SYNCED', 'FAILED', name='expensestatus'), nullable=False),
        sa.Column('freee_deal_id', sa.Integer(), nullable=True),
    ...
    op.create_index('ix_expenses_date_status', 'expenses', ['expense_date', 'status'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_journal_entries_freee_deal_id'), table_name='journal_entries')
    ...
    op.drop_index(op.f('ix_account_item_mappings_active'), table_name='account_item_mappings')
    op.drop_index(op.f('ix_account_item_mappings_mapping_type'), table_name='account_item_mappings')
