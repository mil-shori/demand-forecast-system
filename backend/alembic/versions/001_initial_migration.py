...
def upgrade() -> None:
    ...
    op.create_table('orders',
        ...
        sa.Column('order_type', sa.Enum('SUBSCRIPTION', 'ONEOFF', name='ordertype'), nullable=False),
        ...
        sa.Column('channel', sa.Enum('WEB', 'MOBILE', 'PHONE', 'OTHER', name='channel'), nullable=False),
    ...
    op.create_index('ix_orders_date_type', 'orders', ['order_date', 'order_type'], unique=False)
    op.create_index('ix_orders_user_date', 'orders', ['user_id', 'order_date'], unique=False)
    ...
    op.create_table('products',
        ...
        sa.Column('base_price', sa.Decimal(precision=8, scale=2), nullable=True),
    ...
    op.create_index(op.f('ix_products_is_set_item'), 'products', ['is_set_item'], unique=False)
    ...
    op.create_index('ix_products_brand_category', 'products', ['brand', 'category'], unique=False)
    ...
    op.create_index('ix_subscriptions_next_delivery', 'subscriptions', ['next_delivery_date', 'status'], unique=False)
    ...
    op.create_index('ix_user_profiles_segment', 'user_profiles', ['customer_segment'], unique=False)
    ...
    op.create_table('order_items',
        ...
        sa.Column('discount_rate', sa.Decimal(precision=3, scale=2), nullable=True),
    ...
    op.create_index('ix_order_items_sku_date', 'order_items', ['sku', 'created_at'], unique=False)
    ...
    op.create_table('inventory_snapshots',
        ...
        sa.ForeignKeyConstraint(['sku'], ['products.sku'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('snapshot_date', 'sku', name='uq_inventory_snapshots_date_sku')
    )
    ...
    op.create_index('ix_inventory_snapshots_date_sku', 'inventory_snapshots', ['snapshot_date', 'sku'], unique=False)
    ...
    op.create_table('product_sets',
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['component_sku'], ['products.sku'], ),
        sa.ForeignKeyConstraint(['set_sku'], ['products.sku'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('set_sku', 'component_sku', name='uq_product_sets_sku_component')
    )
    op.create_index('ix_product_sets_set_sku', 'product_sets', ['set_sku'], unique=False)
    
    # Add foreign key constraint to orders table
    op.create_foreign_key(None, 'orders', 'subscriptions', ['subscription_id'], ['subscription_id'])
