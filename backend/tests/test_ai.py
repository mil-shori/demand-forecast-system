...
def test_suggest_requires_api_key(client):
    response = client.post(
        "/api/v1/accounting/ai/suggest-account-item",
        json={
            "category": "交通費",
            "description": "タクシー代",
        },
...
def test_suggest_account_item(client, db_session, monkeypatch):
    ...
    messages = _stub_client(
        monkeypatch,
        parsed=AccountItemSuggestion(
            account_item_id=30,
            account_item_name="旅費交通費",
            confidence=0.95,
            reason="タクシー代は旅費交通費に該当します。",
        ),
    )
...
def test_monthly_summary_requires_api_key(client):
    response = client.get(
        "/api/v1/accounting/ai/monthly-summary", params={"year": 2026, "month": 5}
    )
    assert response.status_code == 503
...
@respx.mock
def test_expense_sync_falls_back_to_default_when_ai_fails(
